        ...
        {hoveredPoint && (
          <div style={{
            ...
            <div style={{ fontSize: '0.85rem', fontWeight: 800, marginBottom: '6px', whiteSpace: 'nowrap' }}>
              {hoveredPoint.data.homeTeam} vs {hoveredPoint.data.awayTeam}
            </div>
...
export default function Profile({ user, setUser }) {
  ...
  const fetchProfileData = async () => {
    try {
      ...
      // 4. Fetch rank history
      const historyRes = await fetch(`/api/leaderboard/history/user/${user.id}`, {
        headers: { 'Authorization': `Bearer ${token}` }
      });
      let historyData = [];
      if (historyRes.ok) {
    ...
    <div className="animate-fade-in" style={{ maxWidth: '1000px', margin: '0 auto', paddingBottom: '40px' }}>
      ...
      {loading ? (
        ...
        <>
          ...
          {/* User Rank Progression History Chart */}
