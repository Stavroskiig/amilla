...
public class StatsService {
    ...
    public GlobalStatsDto getGlobalStats() {
        ...
        calculateMatchSuperlatives(stats, matches, predictions);
    ...
    private void calculatePredictionMatrix(GlobalStatsDto stats, List<Match> matches, List<Prediction> predictions, List<User> users) {
        ...
        List<Match> lockedMatches = matches.stream()
                .filter(m -> m.getKickoffTime() != null && now.isAfter(m.getKickoffTime().minus(5, java.time.temporal.ChronoUnit.MINUTES)))
        ...
        for (Match m : lockedMatches) {
            ...
            // Get all predictions for this match, sorted by updatedAt so latest overwrites earlier ones
            List<Prediction> matchPreds = predictions.stream()
                    .filter(p -> p.getMatchId().equals(m.getId()))
                    .sorted(Comparator.comparing(p -> p.getUpdatedAt() == null ? java.time.Instant.MIN : p.getUpdatedAt()))
                    .collect(Collectors.toList());
    ...
    private Map<String, Double> calculateChampionDistribution(List<LongTermPrediction> preds) {
        if (preds.isEmpty()) return Collections.emptyMap();
        Map<String, Long> counts = preds.stream()
                .filter(p -> p.getPredictedChampionTeam() != null)
                .collect(Collectors.groupingBy(LongTermPrediction::getPredictedChampionTeam, Collectors.counting()));
    ...
    private String calculateMostCommonScoreline(List<Prediction> preds) {
        ...
        Map<String, Long> counts = preds.stream()
                .map(p -> p.getPredictedHomeScore() + " - " + p.getPredictedAwayScore())
                .collect(Collectors.groupingBy(s -> s, Collectors.counting()));
        
        return counts.entrySet().stream()
                .max(Map.Entry.comparingByValue())
                .map(e -> e.getKey() + " (" + e.getValue() + " picks)")
                .orElse("N/A");
    }
    ...
    private void calculateMatchSuperlatives(GlobalStatsDto stats, List<Match> matches, List<Prediction> predictions) {
        ...
        for (Match m : finishedMatches) {
            ...
            double avgPts = matchPreds.stream().mapToInt(Prediction::getPointsEarned).average().orElse(0.0);
    ...
    private void calculateHallOfFame(GlobalStatsDto stats, List<User> users) {
        ...
        User consistent = users.stream()
                .max(Comparator.comparingInt(u -> u.getLongestStreak() != null ? u.getLongestStreak() : 0))
                .orElse(null);
    ...
    private void calculateGlobalAverages(GlobalStatsDto stats, List<Prediction> predictions, List<User> users) {
        ...
        double avgPoints = predictions.stream()
        ...
        long totalCorrect = users.stream().mapToLong(u -> u.getCorrectOutcomes() != null ? u.getCorrectOutcomes() : 0).sum();
        
        long totalFinishedPreds = predictions.stream().filter(p -> p.getUpdatedAt() != null && p.getPointsEarned() >= 0).count(); // rough estimate of finished
        // A better way is just to count the points:
        // 5 points = exact, 2 points = correct result
        
        long exact = predictions.stream().filter(p -> p.getPointsEarned() >= 5).count();
        long correct = predictions.stream().filter(p -> p.getPointsEarned() > 0 && p.getPointsEarned() < 5).count();
        long miss = predictions.stream().filter(p -> p.getPointsEarned() == 0).count();
        // note: miss will include scheduled matches right now if they have pointsEarned=0.
        // We should only count predictions for matches that are FINISHED.
        
        List<String> finishedMatchIds = matchRepository.findAll().stream()
            .filter(m -> "FINISHED".equalsIgnoreCase(m.getStatus()))
            .map(Match::getId)
            .collect(Collectors.toList());
            
        List<Prediction> finishedPreds = predictions.stream()
            .filter(p -> finishedMatchIds.contains(p.getMatchId()))
            .collect(Collectors.toList());

        long exactF = finishedPreds.stream().filter(p -> p.getPointsEarned() >= 5).count();
        long correctF = finishedPreds.stream().filter(p -> p.getPointsEarned() > 0 && p.getPointsEarned() < 5).count();
        long missF = finishedPreds.stream().filter(p -> p.getPointsEarned() == 0).count();
