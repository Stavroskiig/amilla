...
public class StatsService {
    ...
    private void calculatePredictionMatrix(GlobalStatsDto stats, List<Match> matches, List<Prediction> predictions,
        ...
        List<Match> lockedMatches = matches.stream()
                .filter(m -> m.getKickoffTime() != null
                        && now.isAfter(m.getKickoffTime().minus(5, java.time.temporal.ChronoUnit.MINUTES)))
        ...
        for (Match m : lockedMatches) {
            ...
            List<Prediction> matchPreds = predictions.stream()
                    ...
                    .sorted(Comparator
                            .comparing(p -> p.getUpdatedAt() == null ? java.time.Instant.MIN : p.getUpdatedAt()))
                    .collect(Collectors.toList());
    ...
    private String calculateMostCommonScoreline(List<Prediction> preds) {
        ...
        return counts.entrySet().stream()
                ...
                .map(e -> e.getKey() + " (" + e.getValue() + " επιλογές)")
                .orElse("N/A");
    }

    private void calculateMatchSuperlatives(GlobalStatsDto stats, List<Match> matches, List<Prediction> predictions, int totalUsers) {
        ...
        // Only set biggest upset if we have more than one finished match (to avoid duplicating the same match)
    ...
    private void calculateGlobalAverages(GlobalStatsDto stats, List<Prediction> predictions, List<User> users) {
        ...
        double avgPoints = finishedPreds.isEmpty() ? 0.0 : finishedPreds.stream()
        ...
        long totalCorrect = users.stream().mapToLong(u -> u.getCorrectOutcomes() != null ? u.getCorrectOutcomes() : 0)
                .sum();
